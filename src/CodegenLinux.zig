...
pub fn build(self: @This(), path: []const u8, ir: *Ir) ![]const u8 {
    ...
    for (ir.functions.items) |*function| {
        try buildBody(w, function.name, ir, &function.body);
    }
    try buildBody(w, "main", ir, &ir.body);
...
pub fn buildBody(w: *std.Io.Writer, name: []const u8, ir: *Ir, body: *Ir.Body) !void {
    try write(w,
        \\ {s}:
        \\    push rbp
        \\    mov rbp, rsp
    ...
    const stackSize = ir.getStackSize(body);
    ...
    // assign value
    for (body.operations.items) |operation| {
        switch (operation) {
            .label => |label| {
                try w.print("{s}: \n", .{label});
            },
